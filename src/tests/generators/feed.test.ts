...
describe("ƒ feed()", function () {
    it(`should return an array of the first prime numbers greater than a multiple of 1000`, function () {
        const primes = Easy.prime(1000).toIterator();
        const result = Easy.range({ from: 1000, to: 10000, step: 1000 }).feed(primes).toArray();
    ...
    it(`should return an array of the first fibonacci numbers greater than a multiple of 10`, function () {
        const fibonacci = Easy.fibonacci(10).toIterator();
        const result = Easy.range({ from: 10, to: 100, step: 10 })
            .feed(fibonacci)
            .toArray();
